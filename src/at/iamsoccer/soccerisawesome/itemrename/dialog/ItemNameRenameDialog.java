...
public class ItemNameRenameDialog {
    ...
    private static final NamespacedKey INPUT_DATA_KEY = NamespacedKey.fromString("rename:item_name");

    private ItemNameRenameDialog() {
    }

    private static final DialogAction applyAction = DialogAction.customClick(ItemNameRenameDialog::onApply, UNLIMITED_CALLBACK_OPTIONS);
    private static final DialogAction previewAction = DialogAction.customClick(ItemNameRenameDialog::onPreview, UNLIMITED_CALLBACK_OPTIONS);
    ...
    private static void applyLore(Player player, String input, ItemStack item) {
        if (input.isBlank()) {
            ...
            item.setData(DataComponentTypes.ITEM_NAME, parseInput(player, input));
        }
    }
