...
public class ItemLoreRenameDialog {
    ...
    private static final NamespacedKey INPUT_DATA_KEY = NamespacedKey.fromString("rename:lore");

    private ItemLoreRenameDialog() {
    }

    private static final DialogAction applyAction = DialogAction.customClick(ItemLoreRenameDialog::onApply, UNLIMITED_CALLBACK_OPTIONS);
    private static final DialogAction previewAction = DialogAction.customClick(ItemLoreRenameDialog::onPreview, UNLIMITED_CALLBACK_OPTIONS);

    public static DialogLike create(Player player) {
        var item = player.getInventory().getItemInMainHand();

        final String text;
        if (item.getPersistentDataContainer().has(INPUT_DATA_KEY, PersistentDataType.LIST.strings())) {
            text = item.getPersistentDataContainer().get(INPUT_DATA_KEY, PersistentDataType.LIST.strings()).stream().collect(Collectors.joining("\n"));
        } else {
            text = "";
        }
    ...
    private static @NonNull Dialog getDialog(Player player, ItemStack itemStack, String text) {
        ...
        var lore = parseInput(player, text);
        var item = itemStack.clone();
        applyLore(player, text, item);
        var comp = concatComponents(lore);
    ...
    private static void onApply(DialogResponseView response, Audience audience) {
        ...
        // TODO: make sure it only replaces rename lore lines
        applyLore(player, input, item);
        item.editPersistentDataContainer(pdc -> {
            if (input.isBlank()) {
                pdc.remove(INPUT_DATA_KEY);
            } else {
                pdc.set(INPUT_DATA_KEY, PersistentDataType.LIST.strings(), Arrays.stream(input.split("\n")).toList());
            }
        });
    }
    ...
    private static void applyLore(Player player, String input, ItemStack item) {
        ...
        if (input.isBlank()) {
            ...
            item.setData(DataComponentTypes.LORE, ItemLore.lore()
                .lines(parseInput(player, input))
                .build()
            );
        }
    }

    private static @NonNull List<Component> parseInput(Player player, String input) {
        return Arrays.stream(input.split("\n"))
            .map(t -> ItemRenameModule.serializerFor(player).deserialize(t)
